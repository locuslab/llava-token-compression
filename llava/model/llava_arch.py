...
class LlavaMetaModel:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None, **kwargs):
        ...
        self.config.mm_vision_output_text_embedding_size = model_args.mm_vision_output_text_embedding_size
        ...
        if getattr(self, 'mm_projector', None) is None:
            self.mm_projector = build_vision_projector(self.config, self.vision_tower.config)
        ...
        if pretrain_mm_mlp_adapter is not None:
            if os.path.isdir(pretrain_mm_mlp_adapter):
                pretrain_mm_mlp_adapter_path = os.path.join(pretrain_mm_mlp_adapter, "mm_projector.bin")
            else:
                pretrain_mm_mlp_adapter_path = pretrain_mm_mlp_adapter
            mm_projector_weights = torch.load(pretrain_mm_mlp_adapter_path, map_location='cpu')
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if type(images) is list or images.ndim == 5:
            if self.get_model().config.mm_vision_token_compression_type == 'quecc':
            ...
            if getattr(self.get_model().config, 'mm_vision_token_compression_type', None) == 'quecc':
                b = input_ids.size()[0]
                ...
                for k,idx in zip(row_indices, split_idx):
                    rows_with_ids.append(k)
                    main_text_ids.append(input_ids[k][idx+1:])
                    
                main_text = self.tokenizer.batch_decode(main_text_ids, skip_special_tokens=True)
                query_text = []
                idx = 0
                for cur_row_idx, text in enumerate(main_text):
                    while idx < rows_with_ids[cur_row_idx]:
                        print("No input_ids split")
                        query_text.append("image")
                        idx += 1                        
    ...
    def initialize_vision_tokenizer(self, model_args, tokenizer):
        ...
        if model_args.mm_use_im_start_end:
            ...
            if model_args.pretrain_mm_mlp_adapter:
                if os.path.isdir(model_args.pretrain_mm_mlp_adapter):
                    pretrain_mm_mlp_adapter_path = os.path.join(model_args.pretrain_mm_mlp_adapter, "mm_projector.bin")
                else:
                    pretrain_mm_mlp_adapter_path = model_args.pretrain_mm_mlp_adapter
                mm_projector_weights = torch.load(pretrain_mm_mlp_adapter_path, map_location='cpu')
