...
#     --bf16 True \
#     --output_dir $OUTPUT_ROOT/${LLM_VERSION_SAVE_NAME}-pretrain-llm-local-conv-deep-${KERNEL} \
#     --num_train_epochs 1 \
...
# 128 per updated bc 4 grad * 8 bs * 4 gpus
...
deepspeed  --master_port=$(shuf -i 44000-54000 -n 1) llava/train/train_mem.py \
    ...
    --vision_tower openai/clip-vit-large-patch14-336 \
    --pretrain_mm_mlp_adapter $OUTPUT_ROOT/${LLM_VERSION_SAVE_NAME}-pretrain-llm-local-conv-deep-${KERNEL}/mm_projector.bin \
    --mm_projector_type mlp2x_gelu \
    --mm_vision_select_layer -2 \
    --mm_use_im_start_end False \
    --mm_use_im_patch_token False \
    ...
    --bf16 True \
    --output_dir $OUTPUT_ROOT/${LLM_VERSION_SAVE_NAME}-finetune-llm-local-conv-deep-${KERNEL} \
    --num_train_epochs 1 \
